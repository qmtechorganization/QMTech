...
For Docker Compose deployments, the `.env` file must exist on the deployment server next to `docker-compose.yml`; Compose passes it into the application container. It must contain valid `PB_URL`, `PB_EMAIL`, and `PB_PASSWORD` values.
